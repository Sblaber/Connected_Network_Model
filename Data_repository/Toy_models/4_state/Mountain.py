...
# Sweep three energy offsets 
Energy_vec = np.array((0, 0.15, 0.3))

for Energy_index in range(0, 3):

    # Node energies for this case 
    Energy = np.array([0, Energy_vec[Energy_index], Energy_vec[Energy_index], 0])
    beta = 1 / (k_B * T)
    ...
    for Omega_index in range(0, N_omega):
        ...
        # The plotted quantity uses Q = sum(abs(T_ij)) 
        Q[Omega_index] = np.sum(np.abs(T_ij))
        frequency[Omega_index] = omega

        # Note: additional intermediate quantities in the original script were only used
        # to compute integrals that were not plotted or saved. They are removed here.
    ...
    if Energy_index == 0:
        plt.loglog(2 * np.pi * frequency, N_nodes * Q / (4 * Gamma**2.0 * np.pi), 'r', label='$\\Delta E = 0$')
        plt.semilogx(w, np.sum(Q_inv, axis=0), 'k')

    if Energy_index == 1:
        plt.loglog(2 * np.pi * frequency, N_nodes * Q / (4 * Gamma**2.0 * np.pi), 'r--', label='$\\Delta E = 0.15$')
        plt.semilogx(w, np.sum(Q_inv, axis=0), 'k--')

    if Energy_index == 2:
        plt.loglog(2 * np.pi * frequency, N_nodes * Q / (4 * Gamma**2.0 * np.pi), 'r:', label='$\\Delta E = 0.3$ (eV)')
        plt.semilogx(w, np.sum(Q_inv, axis=0), 'k:')
